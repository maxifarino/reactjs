...
export const setHolderProfileError = (error) => {
  return {
    type: types.SET_HOLDER_PROFILE_ERROR,
    error
  };
};

export const setHolderProfile = (hcprofile) => {
  return {
    type: types.SET_HOLDER_PROFILE,
    hcprofile
  };
};

export const setFetchHolderLoading = () => {
  return {
    type: types.SET_HOLDER_PROFILE_FETCHING
  };
};

export const fetchHolderProfile = (id, callback) => {
  return (dispatch, getState) => {
    ...
    let urlQuery = `cf/holderdetail?holderId=${id}`;

    dispatch(setFetchHolderLoading());
    ...
    .then(response => {
      ...
      if(success) {
        ...
        dispatch(setHolderProfile(data[0]));
        ...
        dispatch(setHolderProfileError(errorMsg));
      }
    })
    .catch(() => {
      if (callback) callback(null);
      dispatch(setHolderProfileError(errorConnection));
    });
  };
};
...
export const setAccountProjectsNonArchived = (account)=> {  
  return {
    type: types.SET_ACCOUNT_PROJECT_NON_ARCHIVED,
    account
  }
};