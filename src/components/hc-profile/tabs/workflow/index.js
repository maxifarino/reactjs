...
class Workflow extends React.Component {
  ...
  componentWillMount() {
    getWorkFlow(this.props.global.workflow.hcId, this.props.userProfile.authToken, (err, data) => {
  ...
  renderParameter (component, parameter, idx) {
    ...
    const possibleValues = Utils.getOptionsList("Select an option", parameter.possible_values, 'value', 'value', 'value');
  ...
  render() {
    ...
    const workflowsTableBody = orderedLanguagesList.map((item) => {
      ...
      return {
        ...
        detail: item.detail,
        view: (
          <a className="cell-table-link icon-quick_view" style={style} onClick={(e) => {this.onViewWorkflow(item)}}>
            VIEW WORKFLOW
          </a>
        ),
      };
